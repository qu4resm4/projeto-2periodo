...
function rows_table ($id, $nome, $data_nasc, $sexo, $email, $tel_cel, $tel_fixo, $login) {
    return  <<< row
                <tr>
                    <td>$nome</td>
                    <td>$data_nasc</td>
                    ...
                    <td>$login</td>
                    <td>
                        <span id="$id" class="btnExcluir ms-3">
    ...
    function response($registros, $conexao){
        if ($registros) {
            ...
            while ($linha = mysqli_fetch_assoc($registros)) {
                ...
                echo rows_table($linha["id"], $linha["nome"], $linha["data_nasc"], $linha["sexo"], $linha["email"], $linha["tel_cel"], $linha["tel_fixo"], $linha["login"]);
            }
        } else {
